...
import {
  ...
  type DeveloperForwardAggregateEvent,
  aggregateCounterKey,
  foldAggregateCounters,
  sendDeveloperForwardAggregate,
  toAggregateIncrement,
  validateDeveloperForwardAggregateEvent
} from "@/lib/developer-forward/aggregate";
import {
  GA4_CONFIG_COMMAND,
  TF_EVENT_NAMES,
  TF_EVENT_PREFIX,
  ...
  type DeveloperForwardEventName,
  type DeveloperForwardEventProperties,
  type DeveloperForwardRefusal,
  analyticsConsentGranted,
  bufferedDeveloperForwardEventCount,
  flushDeveloperForwardTelemetry,
  ga4ConfigMarkerPresent,
  measurementIdIsSet,
  resetDeveloperForwardTelemetryForTests,
  trackLite,
  trackDeveloperForward,
  validateDeveloperForwardEvent
} from "@/lib/developer-forward/telemetry";

/**
 * The Developer Forward Lite telemetry substrate (plan §8, §14; PRIVACY_ANALYTICS.md;
 * gate Q-E `Q_E_AGGREGATE_TELEMETRY_SCOPE.BEN_APPROVED.json`).
 *
 * WHY THIS FILE IS MOSTLY NEGATIVE ASSERTIONS. `lib/developer-forward/telemetry.ts`
 ...
 * already names `lib/developer-forward/telemetry.ts` as the second adapter; they are
 * not duplicated here.
 */
...
function withHarness<T>(
  ...
  // store on this origin and the Developer Forward adapter inherits it rather than
...
function collector(): {
  refusals: DeveloperForwardRefusal[];
  details: string[];
  onRefusal: (refusal: DeveloperForwardRefusal, detail: string) => void;
} {
  const refusals: DeveloperForwardRefusal[] = [];
  const details: string[] = [];
  return {
    refusals,
    details,
    onRefusal: (refusal: DeveloperForwardRefusal, detail: string) => {
...
function developerForwardEvents(harness: Harness): unknown[][] {
...
function asProps(value: unknown): DeveloperForwardEventProperties {
  return value as DeveloperForwardEventProperties;
}
...
test("every allowlisted name carries the df_ prefix — two products, one GA4 stream", () => {
  assert.equal(TF_EVENT_PREFIX, "df_");
...
test("an unlisted event name is refused, including the reference's own unprefixed spellings", () => {
  ...
  for (const name of unlisted) {
    const result = validateDeveloperForwardEvent(name, {});
...
test("a non-string event name is refused rather than coerced", () => {
  for (const name of [null, undefined, 42, {}, ["df_lite_started"]]) {
    const result = validateDeveloperForwardEvent(name, {});
...
test("case_number and from_case_number accept 1-5 and nothing else", () => {
  for (const key of ["case_number", "from_case_number"] as const) {
    for (const value of [1, 2, 3, 4, 5]) {
      assert.equal(validateDeveloperForwardEvent("df_case_reached", { [key]: value }).ok, true, `${key}=${value}`);
    }
    for (const value of [0, 6, -1, 2.5, 100, Number.NaN, Number.POSITIVE_INFINITY, "3", null]) {
      const result = validateDeveloperForwardEvent("df_case_reached", { [key]: value });
...
test("decision_number accepts 1-3 — Case 5 is the only case with three decisions", () => {
  for (const value of [1, 2, 3]) {
    assert.equal(validateDeveloperForwardEvent("df_decision_completed", { decision_number: value }).ok, true);
  }
  for (const value of [0, 4, 11, 1.5, "1", true]) {
    const result = validateDeveloperForwardEvent("df_decision_completed", { decision_number: value });
...
test("app_version accepts the shipped stamp and refuses free text", () => {
  ...
  for (const value of [VERSION_MANIFEST.appVersion, "1.0.0", "2026-09-07", "3"]) {
    assert.equal(validateDeveloperForwardEvent("df_lite_started", { app_version: value }).ok, true, value);
  }
  for (const value of ["v1.1.0", "I decided not to tell the client", "", 1.1, null, {}]) {
    const result = validateDeveloperForwardEvent("df_lite_started", { app_version: value });
...
test("an unlisted property key is refused, whatever its value", () => {
  for (const key of ["option_id", "variant_id", "ship_code", "handle", "session_id", "reflection", "user_id"]) {
    const result = validateDeveloperForwardEvent("df_decision_completed", { [key]: 1 });
...
test("a camelCase spelling of an allowlisted key is refused — snake_case is the wire format, no shim", () => {
  ...
  for (const key of ["caseNumber", "decisionNumber", "fromCaseNumber", "appVersion"]) {
    const result = validateDeveloperForwardEvent("df_case_reached", { [key]: 2 });
    assert.equal(result.ok, false, `${key} must not be accepted`);
    assert.equal(result.ok === false && result.refusal, "unknown-property");
  }
});
...
test("a reference-shaped call — trackLite({ name, caseNumber }) — is refused at runtime too", () => {
  ...
  assert.equal(trackLite, trackDeveloperForward);
  const result = validateDeveloperForwardEvent("case_reached", { caseNumber: 2, appVersion: "1.1.0" });
  assert.equal(result.ok, false);
  assert.equal(result.ok === false && result.refusal, "unknown-event");
});
...
test("props that are not a plain object are refused before any key is read", () => {
  for (const props of [null, "a string", 7, [{ case_number: 1 }], true]) {
    const result = validateDeveloperForwardEvent("df_case_reached", props);
...
test("an extra string key cannot survive validation", () => {
  const result = validateDeveloperForwardEvent("df_decision_completed", {
    case_number: 3,
    decision_number: 1,
    app_version: "1.1.0",
...
test("an explicitly undefined allowlisted key is dropped rather than emitted", () => {
  const result = validateDeveloperForwardEvent("df_case_reached", { case_number: 4, decision_number: undefined });
  assert.equal(result.ok, true);
...
test("the adapter source contains no spread of caller-supplied props", () => {
  ...
  const adapter = codeOnly(readFileSync(path.join(repoRoot, "lib/developer-forward/telemetry.ts"), "utf8"));
...
test("the canary is refused as an event name, as a key, as every value, and as the whole props object", () => {
  ...
  const positions: { label: string; run: (onRefusal: (r: DeveloperForwardRefusal, d: string) => void) => void }[] = [
    {
      label: "event name",
      run: (onRefusal) => trackDeveloperForward(CANARY as DeveloperForwardEventName, {}, { onRefusal })
    },
    {
      label: "property key",
      run: (onRefusal) => trackDeveloperForward("df_lite_started", asProps({ [CANARY]: "x" }), { onRefusal })
    },
    {
      label: "whole props object",
      run: (onRefusal) => trackDeveloperForward("df_lite_started", asProps(CANARY), { onRefusal })
    }
  ];
  for (const key of TF_PROPERTY_KEYS) {
    positions.push({
      label: `value under ${key}`,
      run: (onRefusal) => trackDeveloperForward("df_lite_started", asProps({ [key]: CANARY }), { onRefusal })
    });
  }
  ...
  withHarness({ consent: "granted" }, (harness) => {
    ...
    assert.equal(bufferedDeveloperForwardEventCount(), 0);
    assert.equal(developerForwardEvents(harness).length, 0);
...
test("a refused VALUE is never echoed back in the refusal detail", () => {
  ...
  for (const key of TF_PROPERTY_KEYS) {
    const result = validateDeveloperForwardEvent("df_lite_started", { [key]: CANARY });
    assert.equal(result.ok, false);
    assert.equal(result.ok === false && result.refusal, "invalid-property-value");
...
test("no forbidden payload can pass the validator under ANY allowlisted key", () => {
  for (const forbidden of FORBIDDEN) {
    for (const key of TF_PROPERTY_KEYS) {
      const result = validateDeveloperForwardEvent("df_decision_completed", { [key]: forbidden.value });
  ...
  withHarness({ consent: "granted" }, (harness) => {
    ...
    for (const forbidden of FORBIDDEN) {
      for (const name of TF_EVENT_NAMES) {
        ...
        trackDeveloperForward(name, asProps(forbidden.value), { onRefusal });
        // Under each allowlisted key, and under a key named after the fact itself.
        for (const key of TF_PROPERTY_KEYS) {
          trackDeveloperForward(name, asProps({ [key]: forbidden.value }), { onRefusal });
        }
        trackDeveloperForward(name, asProps({ payload: forbidden.value }), { onRefusal });
      }
    }

    assert.ok(refusals.length > 0, "the drive loop refused nothing, so it proved nothing");
    assert.equal(bufferedDeveloperForwardEventCount(), 0, "a forbidden payload was buffered for a later flush");
    assert.equal(developerForwardEvents(harness).length, 0);
...
test("a whole LiteDataset, and a whole LiteResult, are refused as props", () => {
  ...
  const asWhole = validateDeveloperForwardEvent("df_ship_result_viewed", dataset);
  assert.equal(asWhole.ok, false);
  assert.equal(asWhole.ok === false && asWhole.refusal, "unknown-property");

  const nested = validateDeveloperForwardEvent("df_ship_result_viewed", { app_version: dataset });
  ...
  const asResult = validateDeveloperForwardEvent("df_ship_result_viewed", result);
...
test("the kill switch ships on, and flipping it silences every event", () => {
  ...
  withHarness({ consent: "granted" }, (harness) => {
    harness.runGa4Init();
    const result = trackDeveloperForward("df_lite_started", { app_version: "1.1.0" }, { enabled: false });
...
test("with the measurement ID unset, nothing is sent even for a consenting visitor", () => {
  withHarness({ consent: "granted", measurementId: null }, (harness) => {
    ...
    const result = trackDeveloperForward("df_lite_started", { app_version: "1.1.0" });
...
test("with consent denied, nothing is sent and nothing is buffered", () => {
  withHarness({ consent: "denied" }, (harness) => {
    harness.runGa4Init();
    const result = trackDeveloperForward("df_case_reached", { case_number: 1, app_version: "1.1.0" });
...
test("with no consent choice recorded at all, the gate fails closed", () => {
  withHarness({}, (harness) => {
    ...
    trackDeveloperForward("df_case_reached", { case_number: 1 });
    assert.equal(developerForwardEvents(harness).length, 0);
  });
});
...
test("a refusal reaches the onRefusal hook with its reason, its detail and the event", () => {
  withHarness({ consent: "granted" }, (harness) => {
    harness.runGa4Init();
    const seen: { refusal: DeveloperForwardRefusal; detail: string; event: unknown }[] = [];
    trackDeveloperForward("df_case_reached", asProps({ case_number: 99 }), {
    ...
    assert.equal(seen[0].event, "df_case_reached");
  });
});
...
test("without a hook the refusal is LOUD in development, and the warning carries no refused value", () => {
  ...
  try {
    withHarness({ consent: "granted" }, (harness) => {
      harness.runGa4Init();
      const result = trackDeveloperForward("df_lite_started", asProps({ app_version: CANARY }));
    ...
    assert.match(lines[0], /developer-forward-telemetry/);
...
test("supplying onRefusal REPLACES the console report rather than adding to it", () => {
  ...
  try {
    withHarness({ consent: "granted" }, (harness) => {
      harness.runGa4Init();
      const { refusals, onRefusal } = collector();
      trackDeveloperForward("df_lite_started", asProps({ app_version: CANARY }), { onRefusal });
...
test("nothing is pushed to dataLayer before ga4-init's config marker exists", () => {
  ...
  withHarness({ consent: "granted" }, (harness) => {
    ...
    assert.equal(bufferedDeveloperForwardEventCount(), 1);
...
test("a partially-run ga4-init (js/consent/set, no config yet) is not a flush signal", () => {
  withHarness({ consent: "granted" }, (harness) => {
    ...
    trackDeveloperForward("df_lite_started", { app_version: "1.1.0" });
    assert.equal(ga4ConfigMarkerPresent(), false);
    assert.equal(bufferedDeveloperForwardEventCount(), 1);
    assert.equal(developerForwardEvents(harness).length, 0);
  });
...
test("the buffer drains in order once the marker exists, and lands after the consent default", () => {
  withHarness({ consent: "granted" }, (harness) => {
    trackDeveloperForward("df_lite_started", { app_version: "1.1.0" });
    trackDeveloperForward("df_case_reached", { case_number: 1, app_version: "1.1.0" });
    trackDeveloperForward("df_decision_completed", { case_number: 1, decision_number: 2, app_version: "1.1.0" });
    assert.equal(bufferedDeveloperForwardEventCount(), 3);

    harness.runGa4Init();
    assert.equal(flushDeveloperForwardTelemetry(), 3);
    assert.equal(bufferedDeveloperForwardEventCount(), 0);

    assert.deepEqual(
      developerForwardEvents(harness).map((entry) => entry[1]),
      ["df_lite_started", "df_case_reached", "df_decision_completed"]
    );
...
test("once ga4-init has run, a later event is pushed immediately with only its allowlisted props", () => {
  withHarness({ consent: "granted" }, (harness) => {
    harness.runGa4Init();
    const result = trackDeveloperForward("df_navigation_back", { from_case_number: 4, app_version: "1.1.0" });
    assert.equal(result.sent, true);

    const events = developerForwardEvents(harness);
    assert.equal(events.length, 1);
    assert.equal(events[0][0], "event");
    assert.equal(events[0][1], "df_navigation_back");
...
test("the queued entry is arguments-shaped, exactly as gtag() would have pushed it", () => {
  withHarness({ consent: "granted" }, (harness) => {
    harness.runGa4Init();
    trackDeveloperForward("df_case5_completed", { app_version: "1.1.0" });
...
test("the buffer is bounded at 32 — a page that never loads GA must not accrete a queue", () => {
  withHarness({ consent: "granted" }, (harness) => {
    ...
    assert.equal(bufferedDeveloperForwardEventCount(), 32);

    const overflow = trackDeveloperForward("df_case_reached", { case_number: 2, app_version: "1.1.0" });
    ...
    assert.equal(bufferedDeveloperForwardEventCount(), 32, "the buffer grew past its bound");

    harness.runGa4Init();
    assert.equal(flushDeveloperForwardTelemetry(), 32);
    assert.equal(bufferedDeveloperForwardEventCount(), 0);
    assert.equal(developerForwardEvents(harness).length, 32, "the 33rd event must be dropped, not deferred");
  });
});
...
test("every row's declared properties actually validate for that event", () => {
  ...
  for (const row of TF_EVENT_USE) {
    ...
    const result = validateDeveloperForwardEvent(row.event, props);
...
const VALID_AGGREGATE: DeveloperForwardAggregateEvent = {
...
test("TF_AGGREGATE_ENABLED ships false, as a build-time constant and not an env var", () => {
  ...
  const source = codeOnly(readFileSync(path.join(repoRoot, "lib/developer-forward/aggregate.ts"), "utf8"));
...
test("with the flag off, sendDeveloperForwardAggregate returns immediately and calls nothing", () => {
  const spy = spyTransport();
  const outcome = sendDeveloperForwardAggregate(VALID_AGGREGATE, { enabled: false, transport: spy.transport });
...
test("with the flag FORCED ON, it STILL sends nothing — there is no transport", () => {
  ...
  const outcome = sendDeveloperForwardAggregate(VALID_AGGREGATE, { enabled: true });
  assert.equal(outcome.sent, false);
...
test("with the SHIPPED defaults — no options at all — nothing is sent", () => {
  const outcome = sendDeveloperForwardAggregate(VALID_AGGREGATE);
  assert.equal(outcome.sent, false);
  assert.equal(outcome.sent === false && outcome.reason, "disabled");
});
...
test("Q-E's loophole stays closed: no endpoint, no route, no persistence layer", () => {
  ...
  // to activate Developer Forward aggregate telemetry."
  assert.throws(() => readFileSync(path.join(repoRoot, "app/api/developer-forward/aggregate/route.ts"), "utf8"));
  assert.throws(() => readFileSync(path.join(repoRoot, "app/api/developer-forward/route.ts"), "utf8"));
...
test("no module in lib/developer-forward can originate a request", () => {
  ...
  for (const file of sourceFiles(["lib/developer-forward"])) {
    if (primitives.test(codeOnly(readFileSync(file, "utf8")))) {
      offenders.push(path.relative(repoRoot, file).split(path.sep).join("/"));
    }
  }
  assert.deepEqual(offenders, [], "a Developer Forward module can issue a network request");
});
...
test("the aggregate vocabulary is one event name", () => {
  ...
  const unknown = validateDeveloperForwardAggregateEvent({ ...VALID_AGGREGATE, event: "decision_reflected" });
...
test("aggregate validation refuses extra keys — a spread ledger event cannot ride along", () => {
  for (const extra of ["sessionId", "variantId", "handle", "localTimestamp", "sequence", "text"]) {
    const result = validateDeveloperForwardAggregateEvent({ ...VALID_AGGREGATE, [extra]: "x" });
...
test("aggregate validation refuses missing fields, free text and invented ids", () => {
  assert.equal(validateDeveloperForwardAggregateEvent(VALID_AGGREGATE).ok, true);

  const missing = validateDeveloperForwardAggregateEvent({ event: "decision_choice", decisionId: "C3D1" });
  assert.equal(missing.ok === false && missing.refusal, "missing-field");

  for (const field of ["decisionId", "optionId", "appVersion"] as const) {
    const result = validateDeveloperForwardAggregateEvent({ ...VALID_AGGREGATE, [field]: CANARY });
  ...
  assert.equal(validateDeveloperForwardAggregateEvent({ ...VALID_AGGREGATE, decisionId: "C6D1" }).ok, false);
  assert.equal(validateDeveloperForwardAggregateEvent({ ...VALID_AGGREGATE, optionId: "D" }).ok, false);

  for (const input of [null, "a string", 7, [VALID_AGGREGATE]]) {
    const result = validateDeveloperForwardAggregateEvent(input);
...
test("folding loses order, adjacency and sender cardinality — it is a tally, not a log", () => {
  const events: DeveloperForwardAggregateEvent[] = [
  ...
  const withJunk = foldAggregateCounters([
    ...events,
    { ...VALID_AGGREGATE, sessionId: "s-9f31c2a8" } as unknown as DeveloperForwardAggregateEvent
  ]);
