/**
 * Developer Forward Lite — the ledger, the local sessions and the storage substrate
 * (`lib/developer-forward/ledger.ts`, `lib/developer-forward/session.ts`,
 * `lib/developer-forward/storage.ts`).
 *
 ...
 *     learner's export (`config/developer-forward-lite.v1.json` →
 ...
 *     `content/developer-forward/` instead of a blank screen.
 */
...
import {
  ...
  sessionIdsInLedger,
  shouldRollSession
} from "@/lib/developer-forward/session";
import {
  LITE_DECLARED_KEYS,
  LITE_SCHEMA_VERSION,
  LITE_TOP_LEVEL_FIELDS,
  DEVELOPER_FORWARD_LITE_STORAGE_KEY,
  clearLiteState,
  emptyLiteDataset,
...
import type { LedgerEvent, LiteDataset } from "@/lib/developer-forward/types";
...
test("the emitting module never reaches for the timezone NAME", () => {
  ...
  const source = readFileSync(path.join(repoRoot, "lib/developer-forward/ledger.ts"), "utf8");
  const code = source.replace(/\/\*[\s\S]*?\*\//g, "").replace(/\/\/.*$/gm, "");

  for (const forbidden of ["Intl.", "resolvedOptions", "timeZone", "toISOString", "toLocaleString"]) {
    assert.ok(!code.includes(forbidden), `lib/developer-forward/ledger.ts calls ${forbidden}`);
  }
});
...
test("a session id is never transmitted: no telemetry property would accept one", () => {
  ...
  for (const key of TF_PROPERTY_KEYS) {
    for (const value of shapes) {
      const result = validateDeveloperForwardEvent(eventName, { [key]: value });
  ...
  for (const key of ["session_id", "sessionId", "local_session", "handle", "timestamp"]) {
    const result = validateDeveloperForwardEvent(eventName, { [key]: sessionId });
...
test("a browser that throws on localStorage ACCESS returns a valid empty state", () => {
  withThrowingWindow(() => {
    ...
    // renders the approved pre-start explanation from `content/developer-forward/`
...
test("an unparseable blob resolves to a clean empty dataset, not a repaired one", () => {
  withWindow(fakeStorage({ [DEVELOPER_FORWARD_LITE_STORAGE_KEY]: "{ this is not json" }), () => {
    const read = readLiteState(T0);
...
test("a working browser round-trips the ledger through the one storage key", () => {
  ...
  withWindow(store, () => {
    ...
    assert.deepEqual(
      Object.keys(JSON.parse(store.getItem(DEVELOPER_FORWARD_LITE_STORAGE_KEY) as string)).sort(),
    ...
    assert.equal(store.getItem(DEVELOPER_FORWARD_LITE_STORAGE_KEY), null);
    assert.equal(store.getItem("wys:progress"), "not ours", "a Developer Forward clear touched another product's key");
  });
});
