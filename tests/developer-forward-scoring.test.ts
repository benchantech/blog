...
import {
  ...
  calculateShip,
  displayPercent
} from "@/lib/developer-forward/scoring";
import {
  AGGREGATION_POLICY_ID,
  resolveDimensionState,
  type SignalledAnswer
} from "@/lib/developer-forward/aggregation";
import {
  TERMINAL_STATE_COUNT,
  composeNarrative,
  enumerateDimensionStates,
  narrativeClausesFor
} from "@/lib/developer-forward/narrative";
import { SHIP_PROFILE_KEYS, profileForKey } from "@/lib/developer-forward/profiles";
import { SIGNALS } from "@/content/developer-forward/signals";
import {
  NARRATIVE_CLAUSES,
...
/**
 * Developer Forward Lite — the deterministic core, checked against the shipped
 ...
 * arithmetic. The four things below are the only claims Developer Forward Lite
 ...
 * reason `lib/developer-forward/*` and `content/developer-forward/*` are shaped the way
 * they are.
 *
 ...
 * `DEVELOPER_FORWARD_HANDOFF_DIR`) and FAILS — never skips — when they are absent.
...
const HANDOFF_DIR =
  process.env.DEVELOPER_FORWARD_HANDOFF_DIR ??
  path.resolve(
    repoRoot,
    "..",
    "bct-facelift",
...
function readFixture(file: string): string {
  if (!existsSync(file)) {
    assert.fail(
      `Developer Forward fixture missing: ${file}\n` +
        "The approved handoff is not vendored into this repo. Point " +
        "DEVELOPER_FORWARD_HANDOFF_DIR at TRUST_FORWARD_LITE_CODEX_FINAL_LITE_GATES_RESOLVED_2026-09-07."
    );
  }
