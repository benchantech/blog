...
import { STAMP } from "@/content/trust-forward/stamp/v1-1-0";
import { decisionIndex, type SignalledAnswer } from "@/lib/trust-forward/aggregation";
import {
  ANSWER_EVENT_TYPES,
...
/**
 * Trust Forward Lite — the active-path pointer resolver (`lib/trust-forward/
...
test("exactly two axes ever fall back, on exactly the documented prefixes", () => {
  ...
  // The reachability figure `content/trust-forward/variants.ts` documents.
