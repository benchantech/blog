...
import { professionalSummary, professionalSummaryClauses } from "@/lib/trust-forward/summary";
import {
  DECISION_IDS,
  DIMENSIONS,
  OPTION_IDS,
 ...
 * same rule `tests/trust-forward-scoring.test.ts` follows, for the same reason.
 *
...
const SURFACE_LISTS: Readonly<Record<string, readonly string[]>> = {
  TRUST_FORWARD_RESOLVED_SURFACES: indexModule.TRUST_FORWARD_RESOLVED_SURFACES,
  TRUST_FORWARD_ABSENT_BY_DESIGN: indexModule.TRUST_FORWARD_ABSENT_BY_DESIGN,
  TRUST_FORWARD_UNSOURCED_SURFACES: indexModule.TRUST_FORWARD_UNSOURCED_SURFACES
};
...
function namespaceForModuleFile(context: string, moduleFile: string): Record<string, unknown> {
  const modulePath = `content/trust-forward/${moduleFile}`;
...
/** Every `TODO_`-prefixed export across `content/trust-forward/`, as `file.ts: NAME`. */
function actualTodoExports(): string[] {
  const found: string[] = [];
  for (const [modulePath, namespace] of Object.entries(MODULE_NAMESPACES)) {
    const moduleFile = modulePath.slice("content/trust-forward/".length);
...
test("the three surface lists partition every TODO_ constant exactly once", () => {
  ...
  for (const entry of parsed.filter((candidate) => candidate.destination !== null)) {
    assert.equal(
      entry.list,
      "TRUST_FORWARD_RESOLVED_SURFACES",
  ...
  assert.deepEqual(
    [...seen.keys()].sort(),
    actualTodoExports(),
    "The three surface lists and the TODO_ exports in content/trust-forward/ disagree. Every TODO_ constant must be declared in exactly one of TRUST_FORWARD_RESOLVED_SURFACES, TRUST_FORWARD_ABSENT_BY_DESIGN or TRUST_FORWARD_UNSOURCED_SURFACES, and every declared entry must name a constant that exists."
  );
  ...
  assert.deepEqual(
    ...
    "The TODO_ constants in content/trust-forward/ are no longer the five layer 09 closed. Adding one is allowed, but it must be added to TODO_SURFACES above with the reason it exists; removing one is not."
  );
});
...
test("no surface is unsourced, so a future gap has to be declared on purpose", () => {
  assert.deepEqual(
    [...indexModule.TRUST_FORWARD_UNSOURCED_SURFACES],
    [],
    "TRUST_FORWARD_UNSOURCED_SURFACES is no longer empty. If a surface genuinely has no wording and no ruling, that is a real finding and this assertion is the place to record it — update this test with the reason, do not delete it."
  );
});
 ...
 * as `tests/trust-forward-scoring.test.ts`: a skip would leave green checks
 * standing over nothing verified. Point `TRUST_FORWARD_LAYER09_DIR` at the
 ...
 */
const LAYER09_DIR =
  process.env.TRUST_FORWARD_LAYER09_DIR ??
  ...
  if (!existsSync(full)) {
    assert.fail(
      `Approved layer 09 artifact missing: ${full}\n` +
        ...
        "TRUST_FORWARD_LAYER09_DIR at 09_final-copy-completion-2026-09-07."
    );
  }
 ...
 * `lib/trust-forward/summary.ts` throws rather than returning "" for that
...
test("no composition root supplies SHIP axis end captions", () => {
  ...
  for (const file of roots) {
    ...
    for (const match of source.matchAll(/\b(?:endLabels|axisEndLabels)\s*=\s*\{([^}]*)\}/g)) {
      ...
      const isPassThrough =
        relative === "components/trust-forward/Reveal.tsx" && value === "axisEndLabels";
