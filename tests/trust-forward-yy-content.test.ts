...
import { APPROVED_BLURS, applyApprovedBlurs } from "@/content/trust-forward/yy/approved-blurs";
import {
  EVIDENCE_TAGS,
  EVIDENCE_TAG_INDEX,
...
} from "@/content/trust-forward/yy/evidence-tags";
import {
 ...
 *
 * `docs/trust-forward-compositing-policy.md` records Ben's authorization to
 ...
 * test named `tests/trust-forward-yy-content.test.ts` enforces it. **It did not
 ...
 */
const SOURCE_PATH =
  "/Users/benchan/yy/bct-facelift/YY_REDO_V1/Trust Forward Lite — YY Method Cases 1–5 Implementation Source.md";
...
const FIXTURE = JSON.parse(
  readFileSync(path.join(repoRoot, "content/trust-forward/yy/text-fixture.json"), "utf8")
...
test("the shipped narrative is Ben's source plus EXACTLY the approved blurs", (t) => {
  ...
  assert.deepEqual(
    ...
    "A narrative differs from Ben's source by something other than the six approved blurs. " +
      "Every substitution must be registered in content/trust-forward/yy/approved-blurs.ts and " +
 ...
 * `tests/trust-forward-content.test.ts` asserts that claim is true by requiring
