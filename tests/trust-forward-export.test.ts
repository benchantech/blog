...
import { appendEvent } from "@/lib/trust-forward/ledger";
import {
  SIGNAL_LOOKUP,
  inactiveAnswers,
  latestExactAnswers,
...
import { allReceipts, highlightReceipts } from "@/lib/trust-forward/receipts";
...
import { startLiteDataset } from "@/lib/trust-forward/storage";
import { DECISION_IDS, caseOfDecision } from "@/lib/trust-forward/types";
import type {
  ActivePath,
  DecisionId,
  LiteDataset,
...
import { RECEIPT_PHRASES } from "@/content/trust-forward/receipts";
import { REFLECTION_PLACEMENTS } from "@/content/trust-forward/copy";
import { EXPERIENCE, VERSION_MANIFEST } from "@/content/trust-forward/stamp/v1-1-0";

/**
 * Trust Forward Lite — the export artifacts and the local import mirror
...
test("the JSON export has the declared top-level shape", () => {
  ...
  assert.equal(json.product, "trust-forward-lite");
...
test("validateLiteExport fails closed on each of the five conditions individually", () => {
  ...
  const breakages: readonly {
  ...
  }[] = [
    {
      ...
      break: (artifact) => {
        artifact.product = "trust-forward-full";
      }
    },
    {
  ...
  for (const candidate of [null, undefined, "trust-forward-lite", 1, [], {}]) {
