/**
 * Content-integrity digests for the Developer Forward Lite handoff (plan §7).
 *
 ...
 * only content digest was the raw voice corpus. Developer Forward needs digests for
 ...
 *
 * WHY DEVELOPER FORWARD NEEDS THEM AT ALL, when Watch Your Step needed one:
 * every other source this repo cites is either inside the repo or a document
 * that does not change. The Developer Forward rulings are neither. They arrive in a
...
export interface DeveloperForwardDigest {
...
export const DEVELOPER_FORWARD_DIGESTS = [
  {
  ...
  },
  {
    ...
    artifact: "04_reviewed-implementation-plan/recovered/developer_forward_lite_729_profiles_SHIP_recalculated.csv",
    reason:
...
] as const satisfies readonly DeveloperForwardDigest[];

export const DEVELOPER_FORWARD_DIGEST_VALUES: readonly string[] = DEVELOPER_FORWARD_DIGESTS.map(
  (entry) => entry.digest
);
