/**
 * Developer Forward Lite — the five cases, their scenarios, and their eleven
...
import type { CaseNumber, DecisionId, OptionId } from "@/lib/developer-forward/types";
 ...
 */
export type DeveloperForwardProvenance =
  | "ben_canonical"
 ...
 */
export const CASES_PROVENANCE: DeveloperForwardProvenance = "recovered_prior_authoring";
 ...
 * a learner's screen.
 */
export const AUTHORING_NOTES_PROVENANCE: DeveloperForwardProvenance = "recovered_prior_authoring";
...
export interface LiteCase {
  ...
  provenance: DeveloperForwardProvenance;
}
...
export function caseByNumber(caseNumber: CaseNumber): LiteCase {
  ...
  if (!found) {
    throw new Error(`No Developer Forward Lite case numbered ${caseNumber}.`);
  }
  return found;
}
...
export function decisionById(decisionId: DecisionId): CaseDecision {
  ...
  if (!found) {
    throw new Error(`No Developer Forward Lite decision "${decisionId}".`);
  }
  return found;
}
