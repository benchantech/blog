...
export const accessibilityMediaText = {
  ...
  variants: {
    full:
      "Photographs, screenshots and illustrations ship with text alternatives. The Upwork profile screenshot on the home page carries its headline, its rating and review count, its location and its two totals in alternative text, so what the picture is evidence of can be read rather than only seen. The five drawings that open the Developer Forward Lite cases each carry a description of the scene, because they are placed before the writing they introduce and a reader who cannot see one would otherwise begin the case with less than a reader who can. The brand mark in the header is marked decorative, so a screen reader passes over it instead of announcing a filename. No recording has shipped, so there is no transcript to publish. The place a recording will sit is drawn as a labelled outline that a screen reader announces, and the type behind an invented document cannot be filled in at all without a written alternative that preserves the decision the exercise is asking for without giving away the answer."
  }
} as const satisfies AnyCanonicalText;
