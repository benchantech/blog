# Developer Forward Lite — implementation report

**Branch:** `main`. **Plan:** `docs/developer-forward-lite-implementation-plan.md` (rev 7).
...
- **Routes.** `/developer-forward` (1.75 kB) and `/developer-forward-lite` (28.4 kB), both static. `/tf`
...
- **13 modules** under `lib/developer-forward/`, **11** under `content/developer-forward/`, the v1.1.0
...
- **Digest rule.** Developer Forward cites artifacts in an external versioned bundle, and between the
  ...
  declared registry (`content/developer-forward/digests.ts`) carries the same obligations: artifact,
...
- **All five once-unsourced surfaces are closed by layer 09** (`09_final-copy-completion-2026-09-07`,
  ...
  where the copy went, a ruled one as a decision record. `DEVELOPER_FORWARD_UNSOURCED_SURFACES` is now
...
1. **`components/ui/ActionPill` dropped `onClick` whenever `href` was set.** A caller passing both
   got navigation and no callback. Developer Forward's four gated teaser cards are exactly that: links
   to `/tf` that must also fire `tf_full_developer_forward_clicked`. The event never fired. Fixed in the
