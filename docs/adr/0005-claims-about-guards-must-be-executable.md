...
`docs/developer-forward-compositing-policy.md` stated that two tests enforced the compositing seam.
`lib/developer-forward/yy/types.ts` named the file:

> "`tests/developer-forward-yy-content.test.ts` fails the build if a choice, a THEN, a NOW or a
...
- A registry entry named `content/developer-forward/yy/types.ts`, which lives in `lib/`, not `content/`.
...
2. A module claiming to be governed elsewhere must be **actually referenced** by the file it names.
   `tests/developer-forward-content.test.ts` now enforces this for the YY subtree — it caught
...
- Delegation check: live, in `tests/developer-forward-content.test.ts`.
