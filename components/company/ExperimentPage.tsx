...
export function ExperimentPage({ experiment }: { experiment: CompanyExperiment }) {
  return <article className={styles.page}>
    ...
    <section className={cx(styles.section, styles.tint)}><div className={styles.inner}><p className={styles.eyebrow}>Evidence</p><ul className={styles.list}>{experiment.evidence.map((item) => <li key={item}>{item}</li>)}</ul></div></section>
