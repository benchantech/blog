...
/**
 * `/developer-forward`'s completion switch (PRIVACY_ANALYTICS.md, final line;
 ...
 *
 * > `/developer-forward` may inspect local completion to change its own UI, but do
 ...
 * in this file and there must never be one: `lib/developer-forward/telemetry.ts`
 ...
 *
 *  1. NO COPY. Every sentence stays in `content/developer-forward/copy.ts` and
 ...
 *     `content/developer-forward/cases.ts` and `variants.ts` are not dragged into a
 ...
 * first client HTML are byte-identical. That is also what keeps `/developer-forward`
...
import { readLiteState } from "@/lib/developer-forward/storage";
import type { LedgerEvent, LedgerEventType } from "@/lib/developer-forward/types";

/**
 * The ledger events that can only exist downstream of `resultAndExportsPresent`.
 *
 * Every one of these five is emitted by a surface `lib/developer-forward/pointers.ts`
