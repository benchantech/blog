/**
 * Developer Forward Lite — the active-path pointer resolver (plan §6.4).
 *
 ...
 * which is the figure `content/developer-forward/variants.ts` documents — Case 3 at
 ...
 * implies — the changed-scenario notice — lives in `content/developer-forward/
...
import { SIGNALS, carriesDimension } from "@/content/developer-forward/signals";
import {
  accumulatedValue,
  decisionIndex,
  signalledAnswers,
 ...
 *
 * `lib/developer-forward/aggregation.ts` takes the recovered tags as a parameter so
 ...
 * `content/developer-forward/variants.ts` records: `AxisKey` in the shared contract
 ...
 * `content/developer-forward/variants.ts` fails the build instead of quietly
 ...
 *
 * Drives `changedScenarioNotice` in `content/developer-forward/copy.ts` — this
