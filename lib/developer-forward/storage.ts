/**
 * Developer Forward Lite — the local storage substrate (plan §6.2;
 * `config/developer-forward-lite.v1.json` → `storage.key`).
 *
 ...
 * `components/developer-forward/LiteSandbox.tsx`; everything load-bearing lives
 ...
 *     the approved explanation from `content/developer-forward/` instead of
 ...
 * carry — so a Watch Your Step clear cannot touch a Developer Forward ledger and
...
import { newSessionId } from "@/lib/developer-forward/session";
import { DECISION_IDS, type DecisionId, type LedgerEvent, type LiteDataset, type ReflectionDraft } from "@/lib/developer-forward/types";
 ...
 * `lib/wys/browser-keys.ts` registers as `DEVELOPER_FORWARD_STORAGE_KEY` so
 ...
 */
export const DEVELOPER_FORWARD_LITE_STORAGE_KEY = "benchantech:developer-forward-lite:state";
 ...
 * `loaded: false` sentinel in `components/developer-forward/LiteSandbox.tsx`. The
...
export function storageAvailable(): boolean {
  ...
  try {
    const probe = `${DEVELOPER_FORWARD_LITE_STORAGE_KEY}:probe`;
...
export function readLiteState(now: Date = new Date()): LiteReadResult {
  ...
  try {
    const parsed = parseLiteState(store.getItem(DEVELOPER_FORWARD_LITE_STORAGE_KEY), now);
...
export function writeLiteState(input: unknown, now: Date = new Date()): LiteWriteResult {
  ...
  try {
    store.setItem(DEVELOPER_FORWARD_LITE_STORAGE_KEY, JSON.stringify(dataset));
...
export function clearLiteState(now: Date = new Date()): { dataset: LiteDataset; cleared: boolean } {
  ...
  try {
    store.removeItem(DEVELOPER_FORWARD_LITE_STORAGE_KEY);
