/**
 * The Developer Forward Lite first-party aggregate adapter — IMPLEMENTED, DISABLED
 ...
 * persistence/backend infrastructure merely to activate Developer Forward aggregate
 ...
 * `node_modules`. There is no `app/api/developer-forward/aggregate/route.ts` and
 ...
 * `lib/developer-forward/telemetry.ts` has nowhere to put one. The layer-07 ruling
 ...
 * static absence of every request-issuing API over `lib/developer-forward/` one of
...
import { DECISION_IDS, OPTION_IDS, type DecisionId, type OptionId } from "@/lib/developer-forward/types";
 ...
 * GA4 wire property. The snake_case rule governs `trackDeveloperForward` properties
 ...
 */
export interface DeveloperForwardAggregateEvent {
...
export const TF_AGGREGATE_EVENT_NAMES: readonly DeveloperForwardAggregateEvent["event"][] = ["decision_choice"];

/**
 * The transport seam. There is none.
 *
 * This is a value rather than a hardcoded branch inside
 * `sendDeveloperForwardAggregate` so the "there is nowhere to send" half of the
...
export type AggregateValidation =
  | { ok: true; event: DeveloperForwardAggregateEvent }
...
export function validateDeveloperForwardAggregateEvent(input: unknown): AggregateValidation {
  ...
  if (typeof name !== "string" || !TF_AGGREGATE_EVENT_NAMES.includes(name as DeveloperForwardAggregateEvent["event"])) {
 ...
 */
export function aggregateCounterKey(event: DeveloperForwardAggregateEvent): string {
  return `${event.appVersion}|${event.decisionId}|${event.optionId}`;
}

export function toAggregateIncrement(event: DeveloperForwardAggregateEvent): AggregateIncrement {
  return {
...
export function foldAggregateCounters(
  events: readonly DeveloperForwardAggregateEvent[],
  into: Readonly<Record<string, number>> = {}
): Record<string, number> {
  const counters: Record<string, number> = { ...into };
  for (const event of events) {
    const validation = validateDeveloperForwardAggregateEvent(event);
 ...
 */
export function sendDeveloperForwardAggregate(
  event: DeveloperForwardAggregateEvent,
