/**
 * Trust Forward Lite — the local export surface (plan §6.9; `EXPORT_SPEC.md`).
 *
 ...
 * contradiction with a top-level `product: "trust-forward-lite"`, and layer
...
import { LITE_INTRO, RESULT } from "@/content/trust-forward/copy";
 ...
 */
export const LITE_EXPORT_PRODUCT = "trust-forward-lite" as const;
 ...
 * (`01_…/config/trust-forward-lite.v1.json`), governed by the manifest's
 * `shipAxisDefinitionVersion`. They live here rather than in `content/` because
 * `content/trust-forward/` holds no axis-label record: `RESULT.shipExplanation`
...
/**
 * `trust-forward-lite-2026-09-07.md`.
 *
