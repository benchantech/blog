/**
 * Trust Forward Lite — the YY decision ledger and the frozen `YYDecisionRecord`
 ...
 * `lib/trust-forward/ledger.ts` and `lib/wys/telemetry.ts`.
 *
 * WHY THIS IS A SECOND STREAM AND NOT A FEW MORE `LedgerEventType` VALUES.
 * The Lite v1 ledger in `@/lib/trust-forward/ledger` is not extensible to YY
 ...
 *     decision (`config/trust-forward-lite.v1.json` → `timezoneNameStored:
...
import { newSessionId, shouldRollSession } from "@/lib/trust-forward/session";
import { storageAvailable } from "@/lib/trust-forward/storage";
import {
  CHOICE_LABELS,
  type ChoiceLabel,
  type YYCheckpoint,
 ...
 * same reason `lib/trust-forward/ledger.ts` rebuilds from `LEDGER_EVENT_KEYS`
 ...
 * `content/trust-forward/yy/*`, and — the reason that matters — so Ben's THEN
 ...
 * (§3 says so explicitly) — it is a Trust Forward learning step that happens to
...
export function readYYLedger(now: Date = new Date()): YYReadResult {
  ...
  try {
    const raw = storage.getItem(TRUST_FORWARD_YY_STORAGE_KEY);
...
export function writeYYLedger(input: unknown, now: Date = new Date()): YYWriteResult {
  ...
  try {
    storage.setItem(TRUST_FORWARD_YY_STORAGE_KEY, JSON.stringify(ledger));
 ...
 * the v1 Trust Forward dataset and the analytics consent decision, which is a
...
export function clearYYLedger(now: Date = new Date()): { ledger: YYLedger; cleared: boolean } {
  ...
  try {
    storage.removeItem(TRUST_FORWARD_YY_STORAGE_KEY);
