/**
 * The Trust Forward Lite telemetry adapter (plan §8; PRIVACY_ANALYTICS.md;
 ...
 * false`, so Trust Forward layers ON TOP of the same frozen bootstrap that
 ...
 *
 * WHY EVERY NAME IS `tf_`-PREFIXED (plan §8). Both products report into one GA4
 ...
 * static check over `lib/trust-forward/` be a check rather than a promise —
...
export type TrustForwardEventName = (typeof TF_EVENT_NAMES)[number];
...
export type TrustForwardPropertyKey = (typeof TF_PROPERTY_KEYS)[number];
 ...
 */
export interface TrustForwardEventProperties {
  case_number?: number;
 ...
 * construction: `Record<TrustForwardPropertyKey, …>` means adding a key to the
 ...
 */
const PROPERTY_DOMAINS: Record<TrustForwardPropertyKey, (value: unknown) => boolean> = {
...
export type TrustForwardRefusal =
  | "telemetry-disabled"
  | "unknown-event"
...
export type TrustForwardValidation =
  | { ok: true; name: TrustForwardEventName; props: TrustForwardEventProperties }
  | { ok: false; refusal: TrustForwardRefusal; detail: string };

function isEventName(name: unknown): name is TrustForwardEventName {
 ...
 */
export function validateTrustForwardEvent(name: unknown, props: unknown = {}): TrustForwardValidation {
  if (!isEventName(name)) {
  ...
  const clean: TrustForwardEventProperties = {};
...
/**
 * One constant. Flipping it to `false` silences every Trust Forward event
 ...
 * first-paint effect — `tf_lite_started` is exactly that — can run before
 ...
 */
type TrustForwardGtagEventArgs = ["event", TrustForwardEventName, TrustForwardEventProperties];

const pushGtagArguments: (...args: TrustForwardGtagEventArgs) => void = function () {
...
interface BufferedTrustForwardEvent {
  name: TrustForwardEventName;
  props: TrustForwardEventProperties;
}

let buffer: BufferedTrustForwardEvent[] = [];
let pollAttempts = 0;
 ...
 */
export function flushTrustForwardTelemetry(): number {
...
function schedulePoll(): void {
  ...
  const handle = setTimeout(() => {
    pollScheduled = false;
    pollAttempts += 1;
    if (flushTrustForwardTelemetry() === 0 && buffer.length > 0) schedulePoll();
...
export type TrustForwardTrackResult =
  | { sent: true }
  | { sent: false; buffered: true }
  | { sent: false; buffered: false; refusal: TrustForwardRefusal; detail: string };

export interface TrackTrustForwardOptions {
   ...
   */
  onRefusal?: (refusal: TrustForwardRefusal, detail: string, event: unknown) => void;
}

function reportRefusal(
  refusal: TrustForwardRefusal,
  detail: string,
  event: unknown,
  options: TrackTrustForwardOptions
): void {
  ...
  if (process.env.NODE_ENV !== "production" && typeof console !== "undefined") {
    console.warn(`[trust-forward-telemetry] refused: ${refusal} (${detail})`);
  }
}
 ...
 *
 * ```ts
 * trackTrustForward("tf_decision_completed", { case_number, decision_number, app_version });
 * ```
 *
 ...
 */
export function trackTrustForward(
  name: TrustForwardEventName,
  props: TrustForwardEventProperties = {},
  options: TrackTrustForwardOptions = {}
): TrustForwardTrackResult {
  const enabled = options.enabled ?? TF_TELEMETRY_ENABLED;

  const validation = validateTrustForwardEvent(name, props);
  if (!validation.ok) {
  ...
  const drained = flushTrustForwardTelemetry();
 ...
 *
 * So the SIGNATURE is this module's, not the reference's: a `tf_`-prefixed name
 ...
 */
export const trackLite = trackTrustForward;

/** How many events are waiting on `ga4-init`. Inspectable by test. */
export function bufferedTrustForwardEventCount(): number {
 ...
 * internals.
 */
export function resetTrustForwardTelemetryForTests(): void {
  buffer = [];
  pollAttempts = 0;
  pollScheduled = false;
}
 ...
 * learner never clicks — `tf_reflection_shown`, `tf_downstream_scenario_*`,
 * `tf_ship_result_viewed` — would each be wired to a different guess.
 *
 ...
 */
export interface TrustForwardEventUse {
  event: TrustForwardEventName;
  ...
  carries: readonly TrustForwardPropertyKey[];
  /** The funnel question the count answers. Never a question about a person. */
  question: string;
}

export const TF_EVENT_USE: readonly TrustForwardEventUse[] = [
  {
    event: "tf_lite_started",
  ...
  {
    event: "tf_reflection_always_skip",
  ...
  {
    event: "tf_downstream_scenario_invalidated",
  ...
  {
    event: "tf_downstream_scenario_resolved",
  ...
  {
    event: "tf_markdown_export_clicked",
  ...
  {
    event: "tf_full_trust_forward_clicked",
    firesWhen: "The onward CTA to the full Trust Forward is activated.",
    carries: ["app_version"],
 ...
 *    `tf_decision_completed` carries an ordinal and not a `DecisionId` plus an
 *    option: a long enough sequence of fine-grained events reconstructs the
 *    path even when no single event contains an answer.
 *  - **"completion as a learner identity attribute".** `/trust-forward` may
 ...
 *    `tf_case5_completed`, which is attached to nobody.
 *
