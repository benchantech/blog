/**
 * Trust Forward Lite — the SHIP profile lookup, and the phrasing guard that
 ...
 * `@/content/trust-forward/profiles` and the reveal chrome from
 * `@/content/trust-forward/copy`. This module is a lookup and a refusal.
 *
 ...
 * not yet exist in `content/trust-forward/copy.ts`. It is not written here:
...
import { SHIP_PROFILES, type ShipProfile } from "@/content/trust-forward/profiles";
import { RESULT } from "@/content/trust-forward/copy";
import type { ShipResult } from "@/lib/trust-forward/types";
 ...
 * `content/trust-forward/copy.ts` (as e.g. `observedPatternSentence`, verbatim
...
export const TODO_OBSERVED_PATTERN_SENTENCE_MISSING_FROM_CONTENT =
  "content/trust-forward/copy.ts RESULT carries no observed-pattern sentence; " +
