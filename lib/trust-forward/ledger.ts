/**
 * Trust Forward Lite — the append-only local ledger (plan §6.3; ARCHITECTURE.md
 ...
 *     contract in `@/lib/trust-forward/types` is authoritative and is not
 *     redefined here.
 *
 ...
 * `config/trust-forward-lite.v1.json` → `storage.timezoneNameStored: false`).
 ...
 * learner-facing explanation lives in `content/trust-forward/`.
 */
const NO_RANDOM_SOURCE =
  "Trust Forward Lite needs crypto.randomUUID or crypto.getRandomValues to mint a local id; Math.random is not an acceptable substitute.";
